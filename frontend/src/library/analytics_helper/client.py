...
def fetch_price_history(symbol='Bitcoin [BTC]', time_int='1d', time_limit=500, plot_type='line', full_name=True):
    ...
    url = f"{BACKEND_SERVER_ADDRESS}/analyst/coin/price_history/{symbol}?interval={time_int}&limit={str(time_limit)}&plot_type={plot_type}"
...
def fetch_available_coins():
    ...
    url = f"{BACKEND_SERVER_ADDRESS}/analyst/exchange_info/available_coins"
    response = requests.get(url)
    return response.json()
