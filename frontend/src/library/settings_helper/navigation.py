...
def show_status_cards(only_exchange=False) -> None:
    ...
    for k, v in CRYPTO_APIS.items():
        html_txt += status_card(k, v, status_list[i])
        i += 1
...
def show_homepage_status_cards() -> None:
    ...
    statuses = [session_state['backend_status'], session_state["binance_api_status"], session_state["binance_testnet_api_status"],
                session_state["kraken_api_status"], session_state["coinbase_api_status"], session_state["coinbase_sandbox_api_status"] ]
    i = 1
    html_txt = """<div class="section" id="apis">"""
    html_txt += status_card('Backend Server', "https://mdevelopers.com/storage/backend1_a9b6dc2f.png", statuses[0])
    for k, v in CRYPTO_APIS.items():
        html_txt += status_card(k, v, statuses[i])
        i += 1
    html_txt += """</div>"""
    html(html_txt)
...
def api_status_check() -> None:
    if "api_status_check" not in session_state:
        ...
        session_state["binance_api_status"] = status_dict["binance"]  # status_list[0]
        session_state["binance_testnet_api_status"] = status_dict["binance_testnet"]
        session_state["kraken_api_status"] = status_dict["kraken"]
        session_state["coinbase_api_status"] = status_dict["coinbase"]
        session_state["coinbase_sandbox_api_status"] = status_dict["coinbase_sandbox"]
        session_state["mock_exchange_api_status"] = status_dict["mock_exchange"]
